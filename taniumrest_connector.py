...
class TaniumRestConnector(BaseConnector):
        ...
        try:
            ...
            if r.status_code == 404:
                message += "\nThis error usually means the account you are using to interface to Tanium \
                    does not have sufficient permissions to perform this action. "\
                    "See Tanium's documentation for more information on how to change your permissions."
        except Exception:
    ...
    def _make_rest_call(self, endpoint, action_result, verify=True, headers=None, params=None, data=None, json=None,
                        method="get"):
    ...
    def _make_rest_call_helper(self, action_result, endpoint, verify=True, headers=None, params=None, data=None, json=None,
                            method="get"):
        ...
        ret_val, resp_json = self._make_rest_call(url, action_result, verify=verify,
                                                  headers=headers, params=params, data=data, json=json, method=method)
        ...
        if msg and ("403" in msg or "401" in msg):
            self.debug_print(
                "Refreshing Tanium API and re-trying request to [{0}] because "
                "API token was expired or invalid with error code [{1}]".format(url, msg))
            ret_val = self._get_token(action_result)
            ...
            # Issue seen in Tanium 7.3.314.4103.
            # Sometimes it returns a 404 for a sensor and says that the sensor doesn't exist even though it does
            # A short sleep and resubmit fixes the issue
            self.debug_print(
                "Encountered Tanium `REST Object Not Found Exception: SensorNotFound: The requested sensor was not found` error")
            sleep(5)
            ret_val, resp_json = self._make_rest_call(url, action_result, verify=verify,
                                                      headers=headers, params=params, data=data, json=json, method=method)
    ...
    def _handle_list_questions(self, param):
        ...
        for question in response.get("data", []):
            ...
            if question.get("id") and not param.get('list_saved_questions', False) and (question.get('query_text') not in
                                                                                        question_list):
    ...
    def _execute_action_support(self, param, action_result):  # noqa: 901
        ...
        ret_val, distribute_seconds = self._validate_integer(
            action_result, param.get('distribute_seconds'), DISTRIBUTE_SECONDS_KEY)
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        ...
        if parameter_definition and len(parameter_definition.get("parameters")) != 0:
            ...
            try:
                ...
                return action_result.set_status(phantom.APP_ERROR,
                    "Error while parsing the 'package_parameter' field. Error Code: {0}. Error Message: {1}".format(
            ...
            if group_as_obj:
                ...
                if not response_data:
                    return action_result.set_status(phantom.APP_ERROR, "No group exists with name {}. \
                            Also, please verify that your account has sufficient permissions to access the groups".format(
                                group_name))
        ...
        if not response_data:
            return action_result.set_status(phantom.APP_ERROR, "No action group exists with name {}. \
                    Also, please verify that your account has sufficient permissions to access the action groups".format(
                        action_grp))
        ...
        for i in range(1, max_range):
            ...
            if mr_tested and estimated_total:
                ...
                if wait_for_results_processing and num_results_incomplete > 0:
                    ...
                    self.debug_print("Returning results because 'num_results_complete' ({}) >= "
                                     "'return_when_n_results_available' ({})".format(
            ...
            action_result.set_status(phantom.APP_ERROR, "Error while fetching the results from "
            "the Tanium server in '{}' expire seconds. Please try increasing the timeout value".format(timeout_seconds))
            return None
    ...
    def _handle_run_query(self, param):
        ...
        ret_val, return_when_n_results_available = self._validate_integer(action_result, param.get(
            'return_when_n_results_available'), RETURN_WHEN_N_RESULTS_AVAILABLE_KEY, True)
        if phantom.is_fail(ret_val):
            return action_result.get_status()

        # Integer validation for 'wait_for_n_results_available' action parameter
        ret_val, wait_for_n_results_available = self._validate_integer(action_result, param.get(
            'wait_for_n_results_available'), WAIT_FOR_N_RESULTS_AVAILABLE_KEY, True)
        if phantom.is_fail(ret_val):
            return action_result.get_status()

        if return_when_n_results_available and wait_for_n_results_available and (
                                               return_when_n_results_available < wait_for_n_results_available):
            return action_result.set_status(phantom.APP_ERROR,
                                            "Please provide 'return_when_n_results_available' "
                                            "greater than or equal to 'wait_for_n_results_available'")
        ...
        if is_saved_question:
            ...
            if not response_data:
                return action_result.set_status(phantom.APP_ERROR, "No saved question exists with name {}. \
                        Also, please verify that your account has sufficient permissions to access the saved questions".format(
                            query_text))
            ...
            response = self._question_result(timeout_seconds, self._percentage, endpoint, action_result,
                                             wait_for_results_processing, return_when_n_results_available,
    ...
    def _parameterize_query(self, query, action_result):
        ...
        for sensor in sensors:
            ...
            try:
                ...
                error_code, error_msg = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR,
                                                "Error while parsing the 'parameter_definition'. "
    ...
    def _parse_manual_question(self, query_text, action_result, group_name=None):
        ...
        ret_val, response = self._make_rest_call_helper(action_result, TANIUMREST_PARSE_QUESTION, verify=self._verify,
                                                        params=None, headers=None, json=query_to_parse, method="post")
    ...
    def initialize(self):
        ...
        if self._percentage > 100:
            return self.set_status(phantom.APP_ERROR,
                                   "Please provide a valid integer in range of 0-100 in {}".format(RESULTS_PERCENTAGE_KEY))
