...
class TaniumRestConnector(BaseConnector):
    def __init__(self):
        ...
        self._integration_header_value = None
    ...
    def _get_tanium_headers(self, headers=None, include_session=False):
        headers = dict(headers or {})
        headers.update(
            {
                "Content-Type": "application/json",
                TANIUMREST_INTEGRATION_HEADER: self._integration_header_value,
            }
        )
        if include_session:
    ...
    def _parse_comma_separated_values(self, values):
        if not values:
            return []
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            if resp_json.get("text"):
                message = "Error from server. Status Code: {} Data from \
                    server: {}".format(r.status_code, resp_json.get("text"))
            else:
            ...
            if r.status_code == 404:
                ...
                message = f"{message}{permission_error}"
        except Exception:
            message = f"Error from server. Status Code: {r.status_code}. Please provide valid input"
    ...
    def _make_rest_call(self, endpoint, action_result, verify=True, headers=None, params=None, data=None, json=None, method="get"):
        ...
        try:
            ...
            error_message = f"Error connecting to server. Connection Refused from the Server for {endpoint}"
            return RetVal(action_result.set_status(phantom.APP_ERROR, error_message), resp_json)
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR, f"Error occurred while making the REST call to the Tanium server. Error: {error_message}"
                ),
                None,
            )
    ...
    def _make_rest_call_helper(self, action_result, endpoint, verify=True, headers=None, params=None, data=None, json=None, method="get"):
        ...
        headers = self._get_tanium_headers(headers, include_session=True)
        ...
        if msg and ("403" in msg or "401" in msg):
            self.debug_print(
                f"Refreshing Tanium API and re-trying request to [{url}] because API token was expired or invalid with error code [{msg}]"
            )
            ret_val = self._get_token(action_result)
        ...
        if phantom.is_fail(ret_val):
            self.debug_print(f"REST API Call Failure! Failed call to Tanium API endpoint {url} with error code {msg}")
            return action_result.get_status(), None
    ...
    def _handle_test_connectivity(self, param):
        ...
        # make rest call
        ret_val, _ = self._make_rest_call_helper(action_result, TANIUMREST_GET_SAVED_QUESTIONS, verify=self._verify, params=None, headers=None)
    ...
    def _get_response_data(self, response_data, action_result, tanium_content):
        if isinstance(response_data, list):
            if len(response_data) != 1:
                action_result.set_status(phantom.APP_ERROR, f"Error occurred while fetching the {tanium_content}")
                return None
    ...
    def _execute_action_support(self, param, action_result):  # noqa: 901
        ...
        ret_val, distribute_seconds = self._validate_integer(action_result, param.get("distribute_seconds"), TANIUMREST_DISTRIBUTE_SECONDS_KEY)
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            action_result.set_status(phantom.APP_ERROR, f"Error while fetching package details. Error: {error_message}")
        ...
        if parameter_definition and len(parameter_definition.get("parameters")) != 0:
            ...
            try:
                ...
                error_message = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR, f"Error while parsing the 'package_parameter' field. Error: {error_message}")
            ...
            if len(package_parameter) != count_of_params:
                return action_result.set_status(
                    phantom.APP_ERROR, "Please provide all the required package parameters in 'package_parameter' parameter"
                )
            ...
            if invalid_keys:
                return action_result.set_status(
                    phantom.APP_ERROR, "The following key(s) are incorrect: {}. Please provide correct key(s)".format(", ".join(invalid_keys))
                )
        ...
        data["action_group"] = {"id": action_group_id}
    ...
    def _question_result(
        ...
        endpoint,
        action_result,
        wait_for_results_processing=None,
        ...
        for i in range(1, max_range):
            ...
            self.debug_print(f"Checking if Tanium question ID {question_id} has completed and returned all results . . .")
            ...
            if mr_tested and estimated_total:
                ...
                if wait_for_results_processing and num_results_incomplete > 0:
                    ...
                    self.debug_print(
                        f"Tanium question ID {question_id} is {percentage_returned}% done out of {results_percentage}%. Fetching more results . . ."
                    )
                    continue
            ...
            action_result.set_status(
                phantom.APP_ERROR,
                f"Error while fetching the results from the Tanium server in '{timeout_seconds}' expire seconds. \
                    Please try increasing the timeout value",
            )
            return None
    ...
    def _handle_list_processes(self, param):
        ...
        self.debug_print(f"Successfully queried Tanium for 'list_processes' action, got question results id {question_id}")
    ...
    def _handle_run_query(self, param):
        ...
        if return_when_n_results_available and wait_for_n_results_available and return_when_n_results_available < wait_for_n_results_available:
            return action_result.set_status(
                phantom.APP_ERROR, "Please provide 'return_when_n_results_available' greater than or equal to 'wait_for_n_results_available'"
            )
        ...
        if is_saved_question:
            ...
            if not response_data:
                return action_result.set_status(phantom.APP_ERROR, TANIUMREST_RESOURCE_NOT_EXIST.format(query_text, "saved question"))
    ...
    def _handle_get_question_results(self, param):
        ...
        self.save_progress(f"Getting results for question {question_id}")
    ...
    def _handle_create_group(self, param):
        self.save_progress(f"In action handler for: {self.get_action_identifier()}")

        action_result = self.add_action_result(ActionResult(dict(param)))

        group_name = str(param.get("group_name", "")).strip()
        ...
        if not group_name:
            return action_result.set_status(phantom.APP_ERROR, "Please provide a non-empty value in the 'group_name' action parameter")
        ...
        computer_specs = [{"computer_name": computer_name} for computer_name in computer_names]
        computer_specs.extend({"ip_address": ip_address} for ip_address in ip_addresses)

        data = {"name": group_name, "computer_specs": computer_specs}

        ret_val, response = self._make_rest_call_helper(
            action_result, TANIUMREST_CREATE_MANUAL_GROUP, verify=self._verify, params=None, headers=None, json=data, method="post"
        )
    ...
    def _handle_delete_group(self, param):
        ...
        if not group_id and not group_name:
            return action_result.set_status(phantom.APP_ERROR, "Please provide either 'group_id' or 'group_name'")
    ...
    def _parameterize_query(self, query, action_result):
        ...
        self.save_progress(f"Sensors:\n{json.dumps(selects)}")
        # Set param index counter
        self._param_idx = 0
        param_list = query["parameter_values"]

        question_data = {"selects": selects, "question_text": query["question_text"]}
        if "group" in query:
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            self.debug_print(f"Error occurred while converting the sensors. Error: {error_message}")
            return
    ...
    def _create_sensor_dict(self, action_result, sensor, param_list):
        ...
        for key in parameter_keys:
            ...
            parameter = {"key": f"||{key}||", "value": param_list[self._param_idx]}
            parameters.append(parameter)
            self._param_idx += 1

        # Create a new sensor dictionary with valid format
        sensor_dict = {"source_hash": sensor["hash"], "name": sensor_name, "parameters": parameters}
    ...
    def _parse_manual_question(self, query_text, action_result, group_name=None):
        ...
        ret_val, response = self._make_rest_call_helper(
            action_result, TANIUMREST_PARSE_QUESTION, verify=self._verify, params=None, headers=None, json=query_to_parse, method="post"
        )
        self.save_progress(f"Parsed Question:\n{json.dumps(response)}")
    ...
    def _ask_question(
        ...
        self.save_progress(f"Data Posted to /questions:\n{json.dumps(data)}")
        self.save_progress(f"Response from /questions:\n{json.dumps(response)}")
    ...
    def initialize(self):
        ...
        self._integration_header_value = config.get("integration_header_value") or TANIUMREST_DEFAULT_INTEGRATION_HEADER
        ...
        if self._percentage > 100:
            return self.set_status(phantom.APP_ERROR, f"Please provide a valid integer in range of 0-100 in {TANIUMREST_RESULTS_PERCENTAGE_KEY}")
...
if __name__ == "__main__":
    ...
    if username and password:
        try:
            login_url = f"{TaniumRestConnector._get_phantom_base_url()}/login"
